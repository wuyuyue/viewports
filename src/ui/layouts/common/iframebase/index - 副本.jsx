...
class MiniControl extends React.Component{
  ...
  onMouseOverHandler(e){
    var self = this;
    ReactDOM.findDOMNode(self.refs.miniControl).removeEventListener('mouseenter',self.onMouseOverHandler.bind(self),false)
  ...
  onMouseOutHandler(e){
    var self = this;
    ReactDOM.findDOMNode(self.refs.miniControl).addEventListener('mouseenter',self.onMouseOverHandler.bind(self),false)
    //
      ...
      <div  className='miniControl' ref='miniControl'  onMouseEnter={this.state.open? null:this.onMouseOverHandler.bind(this)} onMouseLeave={this.state.open?this.onMouseOutHandler.bind(this):null} >
        {
          this.state.open ?
            <div ref='miniShow' className='row' style={{ height: '100%', margin: 0, padding: 0 }} >
                <input  ref='miniUrl' type="text" value={this.state.url} className="form-control iframeText" id="basic-url" aria-describedby="basic-addon3" onChange={this.handleUrlChange.bind(this)} onKeyDown={this.onKeyDownHandler.bind(this)}/>
                <i className="fas fa-trash" style={{ cursor:'pointer', backgroundColor: 'white', marginLeft: -30, lineHeight: '60px',fontSize: 20 }} onClick={this.remove.bind(this)}></i>
            </div>
              :
            <div ref='miniHide' >
              <i className='fa fa-angle-down' style={{ width: 30, backgroundColor: 'white', fontSize: 20 }}></i>
            </div>
        }
      </div>
    )
    ...
    return [
      <div
        ...
        <div style={{ position: 'relative',width: '100%', height: '100%', boxSizing: 'border-box', pointerEvents: "auto" }}>
          ...
          {
            !url || url.indexOf('about:blank') > -1 ?
               <div className='vhCenter' style={{ padding: "5% 5%", position: 'relative',width: '100%', height: '100%', backgroundColor:'rgba(255,255,255,0.2)'}}>
                 {

                   <div className='withRow' style={{ width: '100%', height: '100%'}}>
                     <div className='iframeText' style={{ width: '40%',  height: '100%', marginRight: '2%' }}>
                        <div className='withColumn' style={{ width: '100%',  height: '100%' }}>
                          <div style={{  height: 60, lineHeight: '60px', fontSize: 60, textAlign:'left', overflow: 'hidden'}}>
                            {self.props.title  ||"about:blank"}
                          </div>
                          <div className='withColumnLeftAuto' style={{ paddingTop: 10, paddingBottom: 10, fontSize: 40, overflow: 'hidden'}}>
                            {unsplashJson.description}
                          </div>
                          <div style={{ height: 50, lineHeight: '50px', fontSize: 50, textAlign: 'right', overflow: 'hidden'}}>
                            {unsplashJson.user.name}
                          </div>
                        </div>
                     </div>
                     <div className='vhCenter withRowLeftAuto' style={{ overflow: 'hidden', border: '1px solid black' }}>
                       <div className='vhCenter'>
                         <img src={unsplashJson.urls.regular} style={{ width: '100%', height: 'auto' }}/>
                       </div>
                     </div>
                   </div>
                 }
